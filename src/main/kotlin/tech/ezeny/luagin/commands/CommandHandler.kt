...
class CommandHandler(private val i18n: I18n, private val scriptManager: ScriptManager) :
    CommandExecutor, TabCompleter {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        if (command.name.equals("luagin", ignoreCase = true)) {
            if (args.isEmpty()) {
                sendHelp(sender)
                return true
            }
            ...
            when (args[0].lowercase()) {
                "reload" -> {
                    if (args.size > 1) {
                        ...
                        val result = scriptManager.reloadScriptByName(scriptName)
                        if (result) {
                        ...
                        val loadedCount = scriptManager.reloadAllScripts()
    ...
    override fun onTabComplete(
        ...
        if (command.name.equals("luagin", ignoreCase = true)) {
            if (args.size == 1) {
                ...
                // 脚本名称补全
                return scriptManager.getLoadedScriptNames().filter {
